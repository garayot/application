...
export default function AdminApplicationDetail() {
  ...
  const IERSection = () => {
    ...
    return (
      <Card>
        <CardHeader>
          <CardTitle>Initial Evaluation (IER)</CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <select
                          {...field}
                          className="w-full h-10 border rounded-md px-3 bg-transparent"
                        >
              ...
              <div className="space-y-4">
                <h3 className="font-bold text-sm uppercase text-slate-500">
                  Qualification Standards & Applicant Levels
                </h3>
                <div className="grid grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl border">
                  <div className="space-y-4">
                    ...
                    />
                    <div
                      className={`p-2 rounded text-center text-sm font-bold ${incEdu < 0 ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                    >
                      Inc: {incEdu}
                      {incEdu < 0 && (
                        <p className="text-[10px] mt-1">BELOW STANDARD</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-4 border-l pl-4">
                    ...
                    />
                    <div
                      className={`p-2 rounded text-center text-sm font-bold ${incTra < 0 ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                    >
                      Inc: {incTra}
                      {incTra < 0 && (
                        <p className="text-[10px] mt-1">BELOW STANDARD</p>
                  ...
                  <div className="space-y-4 border-l pl-4">
                    ...
                    />
                    <div
                      className={`p-2 rounded text-center text-sm font-bold ${incExp < 0 ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                    >
                      Inc: {incExp}
                      {incExp < 0 && (
                        <p className="text-[10px] mt-1">BELOW STANDARD</p>
  ...
  const IESSection = () => {
    ...
    if (app.ies)
      return (
        <CompletedSection
          title="Initial Evaluation Sheet (IES)"
          data={app.ies}
        />
      );
    ...
    return (
      <Card className="mt-8 border-yellow-200 bg-yellow-50/50">
        <CardHeader>
          <CardTitle>Assessment Scoring (IES)</CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <Button type="submit" disabled={createIES.isPending}>
  ...
  const CARSection = () => {
    ...
    if (app.car)
      return (
        <CompletedSection
          title="Comparative Assessment Result (CAR)"
          data={app.car}
        />
      );
    ...
    return (
      <Card className="mt-8 border-purple-200 bg-purple-50/50">
        <CardHeader>
          <CardTitle>Final Deliberation (CAR)</CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        Proceed to Background Investigation?
                      </FormLabel>
                      <FormControl>
                        <select
                          {...field}
      ...
      <div className="max-w-5xl mx-auto pb-20">
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <p className="text-slate-500 text-lg">{app.position.levels}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-6">
            <div>
              <p className="text-sm font-bold">
                Position:{" "}
                <span className="font-normal">{app.position.position}</span>
              </p>
              <p className="text-sm font-bold">
                School Assigned:{" "}
                <span className="font-normal">
                  {(app.ies as any)?.schoolName || "N/A"}
                </span>
              </p>
            </div>
            <div className="text-right">
              <p className="text-sm font-bold">
                ...
                <span className="font-normal">
                  {app.car?.dateOfFinalDeliberation
                    ? new Date(
                        app.car.dateOfFinalDeliberation,
                      ).toLocaleDateString()
                    : "N/A"}
          ...
          <table className="w-full border-collapse border border-slate-900 text-[10px]">
            <thead>
              <tr className="bg-slate-100">
                ...
                <th className="border border-slate-900 p-1 text-center">
                  Rating
                </th>
                ...
                <th className="border border-slate-900 p-1 text-left">
                  Remarks
                </th>
            ...
            <tbody>
              <tr>
                ...
                <td className="border border-slate-900 p-1 text-center">
                  {(app.ies as any)?.pbetLetLptRating}
                </td>
                <td className="border border-slate-900 p-1 text-center">
                  {app.ies?.classObs}
                </td>
                <td className="border border-slate-900 p-1 text-center">
                  {(app.ies as any)?.nonClassObs}
                </td>
                ...
                <td className="border border-slate-900 p-1 capitalize">
                  {app.car?.forBi}
                </td>
                <td className="border border-slate-900 p-1">
                  {app.car?.forAppointment || "N/A"}
                </td>
                <td className="border border-slate-900 p-1">
                  {app.car?.statusOfAppointment || "N/A"}
                </td>
              </tr>
            </tbody>
          </table>
...
function CompletedSection({ title, data }: { title: string; data: any }) {
  return (
    <Card className="bg-slate-50 border-slate-200 opacity-80 hover:opacity-100 transition-opacity">
      <CardHeader>
        <CardTitle className="flex justify-between items-center text-slate-600">
          {title}
